...
class umlip():
  def __init__(self, model=None, fmax=0.001, cell_mask=None) -> None:
    ...
    mask_0D = [False, False, False, False, False, False]

    # for retro compatibility
    self.adjust_vacuum_0D = adjust_vacuum_0D
    ...
    if isinstance(cell_mask, str):
      ...
      elif cell_mask.lower() == "0d":
        self.cell_mask = mask_0D
        self.adjust_vacuum = adjust_vacuum_0D
      else:
  ...
  def init_calculator(self, model=None):
    ...
    # fix annoying memory consuption of tensorflow
    if model_arch == "m3gnet" or model_arch == "grace":
    ...
    if model_arch == "m3gnet":
      ...
      if "orb-v3-conservative-20" in model_path:
        orbff = pretrained.orb_v3_conservative_20_mpa(weights_path=model_path, device=device, precision="float64")
      elif "orb-v3-conservative-inf" in model_path:
        orbff = pretrained.orb_v3_conservative_inf_mpa(weights_path=model_path, device=device, precision="float64")
      elif "orb-v3-direct-20" in model_path:
        orbff = pretrained.orb_v3_direct_20_mpa(weights_path=model_path, device=device, precision="float64")
      elif "orb-v3-direct-inf" in model_path:
        orbff = pretrained.orb_v3_direct_inf_mpa(weights_path=model_path, device=device, precision="float64")
      else:
    ...
    elif model_arch == "fairchem-v2":
      elem_refs = umlip[model]["elem_refs"].replace("$HOME", os.environ["HOME"])

      from fairchem.core import pretrained_mlip, FAIRChemCalculator
      predictor = pretrained_mlip.get_predict_unit(model, device=device, \
              checkpoint_path=model_path, yaml_path=elem_refs)
      return FAIRChemCalculator(predictor, task_name="omat")

    elif model_arch == "sevenn":
      from sevenn.sevennet_calculator import SevenNetCalculator

      return SevenNetCalculator(model_path, modal='mpa', device=device)
    ...
    elif model_arch == "pet-mad":
      from pet_mad.calculator import PETMADCalculator

      return PETMADCalculator(version="1.0", checkpoint_path=model_path, device="cuda")
  ...
  def evaluate(self, struct):
    ...
    if isinstance(stress, np.ndarray):
      stress = stress.astype(np.float32).tolist() # get rid of numpy arrays
  ...
  def relax_structure(self, struct, thick=7.5, relax_cell=True, check_cell=True, 
                      check_connected=True, fix_symmetry=False, pressure=None, fmax_vs_iter=None):
    ...
    class InvalidForcesException(Exception):
      pass
    ...
    def observe_forces(atoms):
      ...
      if fmax_vs_iter is not None:
        fmax_vs_iter.append(np.sqrt(fmax2))
        
      if np.isnan(fmax2) or fmax2 > 10000.0**2:
        raise InvalidForcesException("Error forces are unphysical")
    ...
    for i in range(3):
      ...
      try:
        optimizer.run(fmax=self.fmax/5, steps=500)
      except (InvalidForcesException, AssertionError) as error:
        print(error)
        return None
      ...
      if fmax2 < (self.fmax/5)**2: break # converge better than required
      if fmax2 > 10000.0**2: return None # not converged

    ase_struct = set_calculator(ase_struct)
    forces  = ase_struct.get_forces()
    fmax2  = (forces**2).sum(axis=1).max()
    if fmax2 > self.fmax**2: # Unconverged
      print(f"fmax: {fmax2} > {self.fmax**2}")
      return None
    ...
    stress = ase_struct.get_stress(voigt=False)
    stress = np.array(stress, np.float32) # we use 32 bits in posgres table
...
def adjust_vacuum_1D(ase_struct, thick=12.5, check_cell=True, check_connected=False, tol_angle=TOL):
  ...
  ase_struct.center(vacuum=7.5, axis=(0, 1))

  # check if the resulting structure is "atomically thin"
  pos = ase_struct.get_positions()
  ...
  # construct and hexagonal lattice in xy
  xmean = np.mean(pos[:, 0])
  ymean = np.mean(pos[:, 1])
  rmax = 0.0
  for p in pos:
  ...
  if check_connected and not is_connected(ase_struct, supercell=(1, 1, 2)):
    print("Error: Structure is not connected")
    return None

  return ase_struct


def adjust_vacuum_0D(ase_struct, thick=20.0, check_cell=True, check_connected=False):
  '''Adjust vacuum to 15 Angstrom.'''

  uc = ase_struct.get_cell()

  # Sanity check
  cell_params = uc.cellpar()
  if check_cell and (cell_params[0] > 40.0 or cell_params[1] > 40.0 or cell_params[2] > 40.0):
    print(f"Error: cell vector too large {cell_params[0:3]}")
    return None

  # center positions around (x, y, z) = 1/2
  ase_struct.center(vacuum=7.5, axis=(0, 1, 2))
  ...
  if thick is not None:
    ...
    if max(pos[:,2]) - min(pos[:,2]) > thick:
      print(f"Error: B-AXIS {max(pos[:,2]), min(pos[:,2])}")
      return None

  # check if structure is really zero-dimensional
  if check_connected and not is_connected(ase_struct, supercell=(1, 1, 1)):
    return None
