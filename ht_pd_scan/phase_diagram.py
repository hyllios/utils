...
class PDPlotter:
    ...
    def _get_2d_plot(self, label_stable=True, label_unstable=True,
        ...
        if len(self._pd.elements) == 3:
            ...
            ybuffer = abs(miny) * 0.1
            plt.xlim((-0.1, 1.1))
            plt.ylim((miny - ybuffer, ybuffer))
            center = (0.5, miny / 2)
            plt.xlabel("Fraction", fontsize=35)
            plt.ylabel("Formation energy (eV/atom)", fontsize=35)
        ...
        if self.show_unstable:
            ...
            for entry, coords in unstable.items():
                ...
                if ehull < self.show_unstable:
                    ...
                    if energy_colormap is None:
                        ...
                        plt.plot(coords[0], coords[1], "s", linewidth=3,
                                 ...
                                 markersize=8)
                    print("\\fill[unstablecolor] (", coords[0], ",", coords[1], ") circle (0.012);")
                    if label_unstable:
                ...
                # replot the stable points so that thet are on top
                for x, y in lines:
                    plt.plot(x, y, "ko", **self.plotkwargs)
        ...
        for coords in sorted(labels.keys(), key=lambda x: -x[1]):
            ...
            if coords[1] <= 0.01:
              ...
              anchor = "west"
            print("\\fill[stablecolor] (", coords[0], ",", coords[1], ") circle (0.02);")
            print("\\node[anchor=" + anchor + "] at (", coords[0], ",", coords[1], ") {", latexify(label), "};")
            if label_stable:
